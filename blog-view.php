...
function bp_page_p_edit_c_form(&$qa_content, $id, $comment, $article, $comments, $replysfollows, $in, $errors) {
    ...
    if ($article['replyable']) {
        $replyonoptions[$article['postid']] = qa_lang_html('article/reply_on_q') . qa_html(qa_shorten_string_line($article['title'], 80));
    }
    ...
    foreach ($comments as $othercomment) {
        if ($othercomment['postid'] != $commentid && $othercomment['created'] < $comment['created'] && $othercomment['replyable'] && !$othercomment['hidden']) {
            $replyonoptions[$othercomment['postid']] = qa_lang_html('article/reply_on_a') . qa_html(qa_shorten_string_line(qa_viewer_text($othercomment['content'], $othercomment['format']), 80));
