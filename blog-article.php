    ...
    if (!$formrequested) { // don't show if another form is currently being shown on page
        ...
        if ($article['retagcatbutton']) {
            $buttons['retagcat'] = array(
                ...
                'popup' => qa_lang_html($hascategories ? (qa_using_tags() ? 'bp_lang/retag_cat_popup' : 'bp_lang/recat_popup') : 'bp_lang/retag_popup'
                ),
            );
        }
...
function bp_page_p_post_rules($post, $parentpost = null, $siblingposts = null, $childposts = null) {
    ...
    $rules['answerbutton'] = $post['type'] == 'P' && $permiterror_post_a != 'level' && !$rules['closed'] && (qa_opt('allow_self_comment') || !$rules['isbyuser']);

    $rules['commentbutton'] = ($post['type'] == 'P' || $post['type'] == 'C') && $permiterror_post_c != 'level' && qa_opt($post['type'] == 'P' ? 'comment_on_qs' : 'comment_on_as');
    $rules['commentable'] = $rules['commentbutton'] && !$permiterror_post_c;

    $button_errors = array('login', 'level', 'approve');

    $rules['editbutton'] = !$post['hidden'] && !$rules['closed'] && ($rules['isbyuser'] || (!in_array($permiterror_edit, $button_errors) && (!$post['queued'])));
    $rules['editable'] = $rules['editbutton'] && ($rules['isbyuser'] || !$permiterror_edit);

    $rules['retagcatbutton'] = $post['basetype'] == 'P' && (qa_using_tags() || qa_using_categories()) && !$post['hidden'] && ($rules['isbyuser'] || !in_array($permiterror_retagcat, $button_errors));
    ...
    $rules['flagbutton'] = qa_opt('flagging_of_posts') && !$rules['isbyuser'] && !$post['hidden'] && !$post['queued'] && !@$post['userflag'] && !in_array($permiterror_flag, $button_errors);
    ...
    $rules['reopenable'] = $rules['closed'] && isset($post['closedbyid']) && $permiterror_close_open === false && !$post['hidden'] && ($notclosedbyother || !qa_user_permit_error('permit_close_q', null, $userlevel, true, $userfields));
    ...
    $rules['hideable'] = !$post['hidden'] && ($rules['isbyuser'] || !$post['queued']) && !$permiterror_hide_show_self && ($notclosedbyother || !$permiterror_hide_show);
    ...
    $rules['claimable'] = !isset($post['userid']) && isset($userid) && strlen(@$post['cookieid']) && (strcmp(@$post['cookieid'], $cookieid) == 0) && !($post['basetype'] == 'P' ? $permiterror_post_q : ($post['basetype'] == 'C' ? $permiterror_post_a : $permiterror_post_c));
