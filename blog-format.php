...
function bp_set_up_cat_field(&$qa_content, &$field, $fieldname, $navcategories, $catid, $allownone, $allownosub, $maxdepth = null, $excludecatid = null) {
    ...
    $field['note'] = '<div id="' . $fieldname . '_note">' .
...
function bp_other_to_p_html_fields($article, $userid, $cookieid, $usershtml, $dummy, $options) {
    ...
    if (@$options['whoview']) {
        ...
        if (isset($article['opoints'])) {
            if (@$options['pointsview'])
                $fields['who']['points'] = ($article['opoints'] == 1) ? qa_lang_html_sub_split('main/1_point', '1', '1') : qa_lang_html_sub_split('main/x_points', qa_format_number($article['opoints'], 0, true));
    ...
    if (@$options['flagsview'] && @$article['oflagcount']) {
        $fields['flags'] = ($article['oflagcount'] == 1) ? qa_lang_html_sub_split('main/1_flag', '1', '1') : qa_lang_html_sub_split('main/x_flags', $article['oflagcount']);
    }
...
function bp_post_html_fields($post, $userid, $cookieid, $usershtml, $dummy, $options = array()) {
    ...
    if ($isarticle) {
        ...
        if (@$options['answersview'] && isset($post['ccount'])) {
            ...
            $fields['answers'] = ($post['ccount'] == 1) ? qa_lang_html_sub_split('main/1_comment', '1', '1') : qa_lang_html_sub_split('main/x_comments', qa_format_number($post['ccount'], 0, true));
        ...
        $favoriteclass = '';
        // removed error control operator and added function to check
        // for key in the array
        if (array_key_exists('category', $favoritemap)) {
    ...
    if ($iscomment) {
        ...
        $fields['netvotes_view'] = $netvotes == 1 ? qa_lang_html_sub_split('main/1_vote', $netvoteshtml, '1') : qa_lang_html_sub_split('main/x_votes', $netvoteshtml);
    }
    ...
    if (@$options['whatview']) {
        ...
        if (@$options['whatlink'] && strlen(@$options['p_request'])) {
            $fields['what_url'] = ($post['basetype'] == 'P') ? qa_path_html($options['p_request']) : qa_path_html($options['p_request'], array('show' => $postid), null, null, qa_anchor($post['basetype'], $postid));
        }
    }
    ...
    if (@$options['whoview']) {
        ...
        if (isset($post['points'])) {
            if (@$options['pointsview'])
                $fields['who']['points'] = ($post['points'] == 1) ? qa_lang_html_sub_split('main/1_point', '1', '1') : qa_lang_html_sub_split('main/x_points', qa_format_number($post['points'], 0, true));
...
function bp_setup_notify_fields(&$qa_content, &$fields, $basetype, $login_email, $innotify, $inemail, $errors_email, $fieldprefix = '') {
    ...
    if (empty($login_email)) {
        $fields['notify']['label'] = '<span id="' . $fieldprefix . 'email_shown">' . $labelaskemail . '</span>' .
